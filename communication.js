...
pirate3.onclick = () => {
  playDialog([
    ...
    {speaker:"pirate3",text:"Commençons par les bases."}
  ], startMiniGame1);
};
...
const quiz = [
  ...
  },
  {
    ...
    g:[0,1],
    txt:"Ils développent la visibilité."
  },
  {
    t:"📧 Newsletter",
    ...
    g:[0,1],
    txt:"Elle entretient la relation."
  }
];
    ...
    b.onclick=()=>{
      ...
      if(checkAnswer(s)){
        showNotification(s.txt,()=>{
          qi++;
          qi<quiz.length ? nextQuestion() : afterMiniGame1();
        });
...
function typoExplanation(){
  ...
  const next=document.createElement("button");
  next.textContent="Valider l’identité visuelle";
...
function startMiniGame3(){
  ...
  platforms.forEach(p=>{
    ...
    b.textContent=p[0];
    b.onclick=()=>{ if(!locked.has(b)) selected={b:b,k:p[1]}; };
    left.appendChild(b);
  });
  ...
  targets.forEach(t=>{
    ...
    b.textContent=t[0];
    b.onclick=()=>{
      if(selected && selected.k===t[1] && !locked.has(b)){
        drawLine(svg,selected.b,b);
        locked.add(selected.b);
        locked.add(b);
        selected=null;
        ok++;
        if(ok===3) setTimeout(finish,800);
      }
    };
    right.appendChild(b);
  });
